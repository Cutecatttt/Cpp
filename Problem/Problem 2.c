...
Item* findanItemByID(Item* head, char ID[]) {
  ...
  while(head!= NULL){
      if(strcmp(ID,head->ID)==0) return head;
      head=head->next;
  }
  return NULL; 
}
...
int reduceItemQuantityByID(Item** head, char ID[], int reduceQuantity) {
  ...
  if(strcmp((*head)->ID,ID)==0){
      if((*head)->quantity<=reduceQuantity){
          Item* p = (*head);
          *head = p->next;
          free(p);
      }
      else (*head)->quantity = (*head)->quantity-reduceQuantity;
      return 1;
  }
  Item* p = *head;
  while(p->next!= NULL){
      Item* q = p->next;
      if(strcmp(ID,q->ID)==0){
          q->quantity=q->quantity-reduceQuantity;
          if(q->quantity<=0){
              p->next=q->next;
              free(q);
          }
